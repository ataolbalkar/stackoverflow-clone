...
function addList() {
    ...
    if (tagEditor.document.head.innerHTML === '') {  // If there is no link to stylesheet in the iframe:
        let style = document.head.children[12].cloneNode(true);  // Clone the stylesheet of main document,
...
for (let option of document.getElementById('id_interested_tags').options) {
    ...
    if (event.keyCode === 13) {
        ...
        for (let tag of myTags) {
            if (!(tags.includes(tag.innerText))) {
                ...
                let select = document.getElementById('id_interested_tags').options;
